...
class CancelPendingOrderInput(BaseModel):
    """Input schema for cancel_pending_order tool."""
    order_id: str = Field(
        description="The order id, such as '#W0000000'. Be careful there is a '#' symbol at the beginning of the order id."
    )
    reason: Literal["no longer needed", "ordered by mistake"] = Field(
        description="The reason for cancellation, which should be either 'no longer needed' or 'ordered by mistake'."
    )


def create_cancel_pending_order_tool(data_state: "DataState") -> StructuredTool:
    """
    Create the cancel_pending_order tool with access to data state.

    Args:
        data_state: DataState object containing the database

    Returns:
        StructuredTool for cancelling pending orders
    """
    ...
    def cancel_pending_order(order_id: str, reason: str) -> str:
        ...
        Returns:
            JSON string of order details after cancellation or error message
        """
    ...
    return StructuredTool(
        ...
        func=cancel_pending_order,
        args_schema=CancelPendingOrderInput,
    )